...
def main():
    ...
    for i in range(gpus_n):
        gpu_id = i
        subfolder_l_part = subfolder_l[int(subfolder_n * i / gpus_n) : int(subfolder_n * (i + 1) / gpus_n)]
        process = MyProcess(create_test_png, (model_path, device, gpu_id, opt, subfolder_l_part, save_folder, save_imgs, frame_notation, N_in, PAD,
                    flip_test, end, total_run_time, logger, padding), str(i))
    ...
    #create_test_png(model_path, device, gpu_id, opt, subfolder_l, save_folder, save_imgs, frame_notation, N_in, PAD,
    #                flip_test, end, total_run_time, logger, padding)
    # for subfolder in subfolder_l:
    #
    #     input_subfolder = os.path.split(subfolder)[1]
    #
