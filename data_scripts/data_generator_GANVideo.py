...
# VID_DIR = "G:/Youtube-8M/youtube-8m-id-scrape/"

# GANVideo_new_FineTexture = "/media/wenbo-bao/TOSHIBA EXT/GANVideo_new_fineTexture"

GANVideo_new =  "F:/GANVideo_new/"
# KITTI_VID_DIR = "/media/neil/Neil's 240GB SSD/deep-motion_data/KITTI_RAW/train/"
# KITTI_VID_DIR = "G:/KITTI_data_download/"
# KITTI_VID_DIR_new = "G:/KITTI_data_download_new/"
...
def GANVideo_batch_generator( batch_size, input_frame_size = (3, 128, 128), output_frame_size = (3, 128, 128), data_aug=True):

    raw_im_list = open(os.path.join(GANVideo_new, "im_list_rgb.txt")).read().splitlines()
        ...
        while batch_i < batch_size:
            while 1:
                ...
                if data_aug and random.randint(0, 1):
                    path_pre2 = os.path.join(GANVideo_new, '\\'.join(im_path.split('\\')[:-1]), frame_prefix + str(frame_num_offset+3)+".png")
                    path_pre1 = os.path.join(GANVideo_new, '\\'.join(im_path.split('\\')[:-1]),frame_prefix  + str(frame_num_offset+2)+".png")
                    path_mid  = os.path.join(GANVideo_new, '\\'.join(im_path.split('\\')[:-1]),frame_prefix  + str(frame_num_offset+1)+".png")
                else:
                    path_pre2 = os.path.join(GANVideo_new, '\\'.join(im_path.split('\\')[:-1]), frame_prefix+ str(frame_num_offset+1)+".png")
                    path_pre1 = os.path.join(GANVideo_new, '\\'.join(im_path.split('\\')[:-1]),frame_prefix+  str(frame_num_offset+2)+".png")
                    path_mid  = os.path.join(GANVideo_new, '\\'.join(im_path.split('\\')[:-1]),frame_prefix+  str(frame_num_offset+3)+".png")
            ...
            # plt.figure(4)
            # plt.title("Src Frame")
            # frame = np.concatenate((X[batch_i, :, :, :3],X[batch_i, :, :, 3:6]),axis=0)
...
def main():
    ...
    #     X, y = gen.__next__()
    X0,X1, y = GANVideo_batch_generator(BATCH_SIZE * VALIDATION_MAX_BATCHES,input_frame_size = (3, 128, 128),  output_frame_size = (3, 128, 128)).__next__()
    print(np.mean(np.abs(X0/2+X1/2-y)))
    # np.average()
    np.save("X_val_GANVideo.npy", [X0,X1])
    np.save("y_val_GANVideo.npy", y)
    exit()
