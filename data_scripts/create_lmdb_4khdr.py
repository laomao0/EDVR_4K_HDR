...
parser.add_argument("--dataset", type=str, required=True, help='vimeo90K | REDS | 4KHDR| general (e.g., DIV2K, 291) | DIV2K_demo |test')
...
def HDR(mode):
    ...
    if mode == 'train_sharp':
        ...
        img_folder = "/DATA7_DB7/data/4khdr/data/Dataset/train_540p"
        lmdb_save_path = '/DATA7_DB7/data/4khdr/data/Dataset/{}.lmdb'.format(args.name)
        H_dst, W_dst = 540, 960
    elif mode == 'train_4k':
        img_folder = "/DATA7_DB7/data/4khdr/data/Dataset/train_4k"
        lmdb_save_path = '/DATA7_DB7/data/4khdr/data/Dataset/{}.lmdb'.format(args.name)
        H_dst, W_dst = 540, 960
    ...
    #### read all the image paths to a list
    print('Reading image path list ...')
    all_img_list = data_util._get_paths_from_images(img_folder, args.small)
    keys = []
    for img_path in all_img_list:
        split_rlt = img_path.split('/')
        folder = split_rlt[-2]
...
if __name__ == "__main__":
    if Test == 1:
        test_lmdb('/DATA7_DB7/data/4khdr/data/Dataset/train_4k.lmdb')
    else:
        main()
