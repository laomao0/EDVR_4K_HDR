...
def main():
    ...
    for subfolder in subfolder_l:
        ...
        for img_idx, img_path in enumerate(img_path_l):
            ...
            intPaddingRight = 64 # 32# 64# 128# 256
            intPaddingLeft = 64  # 32#64 #128# 256
            intPaddingTop = 64  # 32#64 #128#256
            intPaddingBottom = 64  # 32#64 # 128# 256
            ...
            for split_j, split_i in itertools.product(splitsY, splitsX):
                ...
                # use self ensamble method

                X0_rot90  = X0.rot90(1, [3,4])
                X0_rot180 = X0.rot90(2, [3,4])
                X0_rot270 = X0.rot90(3, [3,4])

                X0_flip = X0.flip([3])
                X0_rot90_flip = X0_rot90.flip([3])
                X0_rot180_flip =X0_rot180.flip([3])
                X0_rot270_flip = X0_rot270.flip([3])

                X = [X0, X0_rot90, X0_rot180, X0_rot270, X0_flip, X0_rot90_flip, X0_rot180_flip, X0_rot270_flip]
                Y = []
                for index, Xs in enumerate(X):

                    if flip_test:
                        output = util.flipx4_forward(model, Xs)
                    else:
                        output = util.single_forward(model, Xs) # 1 x c x H W

                    # test_output = util.tensor2img(output)
                    # cv2.imwrite(osp.join(save_subfolder, '{}_{}.png'.format(img_name, index)), test_output)

                    Y.append(output)

                Y[0] = Y[0]
                Y[1] = Y[1].rot90(4-1, [2,3])
                Y[2] = Y[2].rot90(4-2, [2,3])
                Y[3] = Y[3].rot90(4-3, [2,3])
                Y[4] = Y[4].flip([2])
                Y[5] = Y[5].flip([2]).rot90(4 - 1, [2, 3])
                Y[6] = Y[6].flip([2]).rot90(4 - 2, [2, 3])
                Y[7] = Y[7].flip([2]).rot90(4 - 3, [2, 3])

                # for i in range(8):
                #     test_output = util.tensor2img(Y[i])
                #     cv2.imwrite(osp.join(save_subfolder, '{}_{}.png'.format(img_name, i)), test_output)
