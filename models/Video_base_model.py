...
class VideoBaseModel(BaseModel):
    ...
    #         self.log_dict['l_pix'] = loss_tmp[0].item()
    #         self.log_dict['ssim_loss'] = loss_tmp[1].item()
    #         self.log_dict['vmaf_loss'] = loss_tmp[2].item()
    ...
    def optimize_parameters(self, step):
        ...
        if self.loss_type == 'msssim':
            self.log_dict['msssim_loss'] = l_pix.item()
        elif self.loss_type == 'ssim':
            self.log_dict['ssim_loss'] = l_pix.item()
        elif self.loss_type == 'cb+ssim':
            self.log_dict['l_pix'] = loss_tmp[0].item()
            self.log_dict['ssim_loss'] = loss_tmp[1].item()
        elif self.loss_type == 'cb+msssim':
            self.log_dict['l_pix'] = loss_tmp[0].item()
            self.log_dict['msssim_loss'] = loss_tmp[1].item()
        else:
            self.log_dict['l_pix'] = l_pix.item()
